...
namespace ModdedControllerSupport.RecipeBrowserPort.UIElements
{
	...
	{
		private static readonly Asset<Texture2D> CloseButtonTexture = ModdedControllerSupport.instance.Assets.Request<Texture2D>("RecipeBrowserPort/UIElements/closeButton", AssetRequestMode.ImmediateLoad);
		...
		public void Unfocus()
		{
			if (focused)
			{
				...
				OnUnfocus?.Invoke();
				// Hide virtual keyboard when unfocusing (if shown)
				try { RecipeBrowserUI.instance?.HideVirtualKeyboard(); } catch { }
			}
		}

		// Allow external callers (like a virtual keyboard) to trigger the Enter action
		...
		public void Focus()
		{
			...
			// If using a controller, show the virtual keyboard
			try
			{
				if (PlayerInput.UsingGamepad || Microsoft.Xna.Framework.Input.GamePad.GetState(Microsoft.Xna.Framework.PlayerIndex.One).IsConnected)
				{
					RecipeBrowserUI.instance?.ShowVirtualKeyboard(this);
				}
			}
			catch { }
			}
		...
		protected override void DrawSelf(SpriteBatch spriteBatch)
		{
			...
			if (focused)
			{
				...
				if (JustPressed(Keys.Enter))
				{
					...
					OnEnterPressed?.Invoke();
					// no-op: preserve context, no behavior change
				}
