			...
			if (real && item.stack > 0)
			{
				...
				if (item2.stack > 0)
				{
					...
					else
					{
						...
						ModdedControllerSupport.instance.Logger.Warn(Language.GetTextValue("Mods.ModdedControllerSupport.RecipeBrowserPort.ItemLostInQuerySlotWarning") + item2.Name);
					}
				}
				item = new Item();
			}
