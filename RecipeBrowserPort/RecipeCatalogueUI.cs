...
namespace ModdedControllerSupport
{
	...
	{
		internal static string RBText(string key, string category = "RecipeCatalogueUI") => ModdedControllerSupport.RBText(category, key);
		...
		internal UIElement CreateRecipeCataloguePanel() {
			...
			Asset<Texture2D> historyBackButtonTexture = ModdedControllerSupport.instance.Assets.Request<Texture2D>("RecipeBrowserPort/UIElements/historyBack", AssetRequestMode.ImmediateLoad);
			...
			Asset<Texture2D> historyForwardButtonTexture = ModdedControllerSupport.instance.Assets.Request<Texture2D>("RecipeBrowserPort/UIElements/historyForward", AssetRequestMode.ImmediateLoad);
			...
			if (ModdedControllerSupport.itemChecklistInstance != null) {
			...
			uniqueCheckbox = new UICycleImage(ModdedControllerSupport.instance.Assets.Request<Texture2D>("Images/uniqueTile") /* Thanks MiningdiamondsVIII */, 2, new string[] { RBText("ShowInheritedRecipes"), RBText("ShowUniqueRecipes") }, 36, 20);
			...
			for (int chestIndex = 0; chestIndex < 1000; chestIndex++) {
				...
				if (chest != null && !Chest.IsLocked(chest.x, chest.y)) {
					...
					if ((chestPosition - Main.LocalPlayer.Center).Length() < itemSearchRange) {
						if (Main.netMode == NetmodeID.SinglePlayer || ModdedControllerSupport.chestContentsAvailable[chestIndex]) {
		...
		private void NearbyIngredientsRadioBitton_OnSelectedChanged(object sender, EventArgs e) {
			...
			ModdedControllerSupport.chestContentsAvailable = new bool[1000];
			for (int chestIndex = 0; chestIndex < 1000; chestIndex++) {
				Chest chest = Main.chest[chestIndex];
				if (chest != null && !Chest.IsLocked(chest.x, chest.y)) {
					Vector2 chestPosition = new Vector2((float)(chest.x * 16 + 16), (float)(chest.y * 16 + 16));
					if ((chestPosition - Main.LocalPlayer.Center).Length() < itemSearchRange) {
						...
						{
							var message = ModdedControllerSupport.instance.GetPacket();
		...
		internal void InvalidateExtendedCraft() {
			...
			while (ModdedControllerSupport.instance.concurrentTasks.TryDequeue(out var _)) ;
			updateNeeded = true;
