...
namespace ModdedControllerSupport
{
	...
	private RecipeBrowserPort.UIElements.VirtualKeyboard virtualKeyboard;
		...
		internal static string RBText(string key, string category = "RecipeBrowserUI") => ModdedControllerSupport.RBText(category, key);

		public RecipeBrowserUI(UserInterface ui) : base(ui)
		{
			instance = this;
			mods = new string[] { "ModLoader" };
		virtualKeyboard = new RecipeBrowserPort.UIElements.VirtualKeyboard();
		// Do not append by default; it will be appended when needed
		}
		...
		// Prepare virtual keyboard UI but don't show until needed
		virtualKeyboard.Deactivate();
			...
			Asset<Texture2D> filterModTexture = ModdedControllerSupport.instance.Assets.Request<Texture2D>("Images/filterMod", AssetRequestMode.ImmediateLoad);
			Asset<Texture2D> filterModColorableTexture = ModdedControllerSupport.instance.Assets.Request<Texture2D>("Images/filterModColorable", AssetRequestMode.ImmediateLoad);
			...
			Asset<Texture2D> closeButtonTexture = ModdedControllerSupport.instance.Assets.Request<Texture2D>("RecipeBrowserPort/UIElements/closeButton", AssetRequestMode.ImmediateLoad);
			...
			HideUnlessInventoryToggle = new UIElements.UICycleImage(ModdedControllerSupport.instance.Assets.Request<Texture2D>("RecipeBrowserPort/UIElements/TickOnOff", AssetRequestMode.ImmediateLoad), 2, new string[] { RBText("AlwaysShow", "FavoritedUI"), RBText("ShowWhenInventory", "FavoritedUI") }, 16, 12);
	...
	internal void ShowVirtualKeyboard(UIElements.NewUITextBox box)
	{
		if (virtualKeyboard == null) return;

		virtualKeyboard.AttachTo(box);
		if (virtualKeyboard.Parent == null)
		{
			Append(virtualKeyboard);
		}
		virtualKeyboard.Activate();
		// Block player input while virtual keyboard active
		try { Main.blockInput = true; } catch { }
	}



	internal void HideVirtualKeyboard()
	{
		if (virtualKeyboard == null) return;
		virtualKeyboard.Deactivate();
		if (virtualKeyboard.Parent != null)
		{
			RemoveChild(virtualKeyboard);
		}
		}
		...
		internal void UpdateFavoritedPanel()
		{
			...
			if (Main.GameUpdateCount > 0) {
				...
				var keys = ModdedControllerSupport.instance.ToggleFavoritedPanelHotKey.GetAssignedKeys();
				if (keys.Count != 0) {
		...
		{
			object result = ModdedControllerSupport.itemChecklistInstance.Call("RequestFoundItems");
			if (result is string)
			{
