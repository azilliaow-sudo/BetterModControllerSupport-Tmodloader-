...
namespace ModdedControllerSupport
{
	...
	internal class CraftPath
	{
		internal class CraftPathNode
		{
			...
			internal void Print(int indent)
			{
				if (!RecipePathTester.print && !RecipePathTester.printResults)
					return;
				ModdedControllerSupport.instance.Logger.Info(new String(' ', indent) + ToString());
				if (children != null) // TODO: Initilize to 0? Leave as null?
					foreach (var item in children)
					{
						if (item == null)
							ModdedControllerSupport.instance.Logger.Info(new String(' ', indent + 4) + "null");
						else
							item.Print(indent + 4);
					}
			}
		...
		internal void Print()
		{
			if (!RecipePathTester.print)
				return;
			ModdedControllerSupport.instance.Logger.Info("Printing CraftPath");
			root.Print(0);
		}
