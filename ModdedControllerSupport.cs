...
using ModdedControllerSupport.RecipeBrowserPort.TagHandlers;
using UIElements = ModdedControllerSupport.RecipeBrowserPort.UIElements;
using TagHandlers = ModdedControllerSupport.RecipeBrowserPort.TagHandlers;

namespace ModdedControllerSupport
{
	
	public class ModdedControllerSupport : Mod
	{


        internal static ModdedControllerSupport instance;
        ...
        {
            // Try ModdedControllerSupport localization first, fall back to RecipeBrowser originals if missing.
            string primaryKey = $"Mods.ModdedControllerSupport.{category}.{key}";
            string fallbackKey = $"Mods.RecipeBrowser.{category}.{key}";

            string value;
            if (args == null || args.Length == 0)
                value = Language.GetTextValue(primaryKey);
            else
                value = Language.GetTextValue(primaryKey, args);

            // Language.GetTextValue returns the key itself when a translation is missing (e.g. "Mods.X.Y").
            // If we detect that, try the RecipeBrowser namespace as a fallback.
            if (value != null && value.StartsWith("Mods."))
            {
                if (args == null || args.Length == 0)
                    value = Language.GetTextValue(fallbackKey);
                else
                    value = Language.GetTextValue(fallbackKey, args);
            }
        ...
        {
            return Language.GetText($"Mods.ModdedControllerSupport.{category}.{key}");
        }
        ...
        public override void PostSetupContent()
        {
            ...
            try
            {
                var extra = ItemID.Sets.GamepadExtraRange;
                var whole = ItemID.Sets.GamepadWholeScreenUseRange;
                if (extra != null)
                {
                    Array a = extra as Array;
                    Type et = a.GetType().GetElementType();
                    for (int i = 0; i < a.Length; i++)
                    {
                        try
                        {
                            if (et == typeof(int)) a.SetValue(1, i);
                            else if (et == typeof(bool)) a.SetValue(true, i);
                            else if (et == typeof(byte)) a.SetValue((byte)1, i);
                        }
                        catch { }
                    }
                }
            ...
            if (!Main.dedServ)
            {
                ...
                UIElements.UICheckbox.checkboxTexture = Assets.Request<Texture2D>("RecipeBrowserPort/UIElements/checkBox");
                UIElements.UICheckbox.checkmarkTexture = Assets.Request<Texture2D>("RecipeBrowserPort/UIElements/checkMark");
                UIHorizontalGrid.moreLeftTexture = Assets.Request<Texture2D>("RecipeBrowserPort/UIElements/MoreLeft");
                UIHorizontalGrid.moreRightTexture = Assets.Request<Texture2D>("RecipeBrowserPort/UIElements/MoreRight");
                UIGrid.moreUpTexture = Assets.Request<Texture2D>("RecipeBrowserPort/UIElements/MoreUp");
                UIGrid.moreDownTexture = Assets.Request<Texture2D>("RecipeBrowserPort/UIElements/MoreDown");
